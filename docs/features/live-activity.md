...
**Does flip to a live "+HH:MM:SS" count-up once over target, matching the in-app ring** — `timerText(_:size:)` branches at render time: `if Date() >= target`, it renders `overtimeText(from:)` (a fresh count-up `Text(timerInterval:)` anchored to `target`, prefixed with `"+"` via SwiftUI's `Text` concatenation) instead of the countdown. Same for `timerColor(_:)`'s olive→amber switch. Neither is a SwiftUI primitive that updates itself — both need an actual redraw (a local/push `update()` call) to re-evaluate the branch.

This turned out to matter in practice, not just in theory: `RoutineSession.tsx`'s per-item switch effect used to call `updateRoutineActivity()` exactly once, when the item became active. A habit left running past its own target with no item switch in between got no further redraw at all — the countdown froze at `00:00` and the color stayed olive indefinitely, confirmed by the user, not merely delayed. The effect now also schedules one extra local `setTimeout` — `activityCrossingTimeoutRef` — timed to fire exactly `projectedMinutes` after the item's virtual start, which re-runs the same update-building logic (factored out as `pushUpdate()`) to push a fresh `ContentState` right at the crossing instant. Skipped for stopwatch items (`projectedMinutes === 0`, no target to cross) and for an item that's already over target when its effect runs (`msUntilTarget <= 0`) — in the latter case the *first* `pushUpdate()` call already renders the over-target branch correctly, nothing further to schedule. Cleared on item switch/unmount alongside the other per-item refs, so at most one crossing timer is ever pending.
...
Like the countdown/color above, this is refreshed on an item switch, a push trigger, and now also the one scheduled crossing-instant `pushUpdate()` described above — not per-second. A routine's projected finish time doesn't need second-level precision on a Lock Screen glance, and this is already far more granular than the "only updates when the app is reopened" gap this feature exists to close.
